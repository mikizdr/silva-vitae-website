import type { ChecklistCardProps, ChecklistSection, CommonMistake } from '../types/titleChecklist';
...
const checklistSections: ChecklistSection[] = [
  ...
  },
  {
    title: '2. Планирање',
    items: [
      'Одредити почетну линију и проверити симетрију резаних плочица',
      'Обележити линије кредом или ласером',
    ],
  },
  {
    ...
    items: [
      ...
      'Убацити дистанцере за једнаке фуге',
    ],
  },
  {
    title: '4. Резање плочица',
    items: [
      'Измерити тачно место реза',
      'Користити резач плочица или брусилицу',
  ...
  },
  {
    title: '5. Фугирање',
    items: [
      'Сачекати сушење лепка (24h)',
      'Нанети фуг масу гуменом шпахтлом',
      'Вишак уклонити влажним сунђером',
    ],
  },
  {
    title: '6. Завршни радови',
    items: [
      'Очистити плочице од остатака фуг масе',
      'Проверити да су све плочице равне и добро залепљене',
      'Уклонити дистанчере',
    ],
  },
];
...
const commonMistakes: CommonMistake[] = [
  [
    'Прескакање хидроизолације',
    'Влага улази у зид, оштећења',
    'Увек нанети хидроизолацију у купатилу',
  ],
  ...
  return (
    <article className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
      ...
      <ul className="space-y-3">
        {items.map((item) => (
          <li key={item}>
            <label className={`${styles.itemLabel} hover:bg-slate-50 dark:hover:bg-slate-800`}>
              <input
                type="checkbox"
                className={`${styles.itemCheckbox} border-slate-300 text-emerald-600 focus:ring-emerald-500 dark:border-slate-600 dark:bg-slate-800 dark:checked:bg-emerald-500`}
              />
              <span className={`${styles.itemText} text-slate-700 dark:text-slate-200`}>
                {item}
              </span>
            </label>
          </li>
    ...
    <>
      <header className="mb-8 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200 sm:p-8 dark:bg-slate-900 dark:ring-slate-800">
        ...
        <h1 className="text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl dark:text-slate-100">
          Checklist за лепљење плочица
        </h1>
        <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
          Практична контролна листа за чист, раван и трајан резултат.
        </p>
      </header>
      ...
      <section className="mt-8 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200 sm:p-6 dark:bg-slate-900 dark:ring-slate-800">
        <h2 className="mb-4 text-lg font-semibold text-slate-900 dark:text-slate-100">
          ⚠️ Честе грешке и како их избегнути
        </h2>
